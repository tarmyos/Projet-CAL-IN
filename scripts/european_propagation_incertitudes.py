...
def propagation_incertitudes_call(S, K, T, r, sigma, dS, dK, dT, dr, dSigma):
    ...
    # l'incertitude ne se fait que sur les paramètres r, sigma et T
    ...
    delta_call = math.sqrt( (dCall_dr * dr)**2 + (dCall_dSigma * dSigma)**2 + (dCall_dT * dT)**2)
...
def propagation_incertitudes_put(S, K, T, r, sigma, dS, dK, dT, dr, dSigma):
    ...
    delta_put = math.sqrt((dPut_dr * dr)**2 + (dPut_dSigma * dSigma)**2 + (dPut_dT * dT)**2)
